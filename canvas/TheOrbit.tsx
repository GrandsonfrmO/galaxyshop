...
const AsteroidField = ({ count = 40 }) => {
  ...
  const data = useMemo(() => {
    return new Array(count).fill(0).map(() => {
      // Chaque astéroïde a sa propre vélocité aléatoire unique
      const velocityX = (Math.random() - 0.5) * 0.015;
      const velocityY = (Math.random() - 0.5) * 0.015;
      const velocityZ = (Math.random() - 0.5) * 0.015;
      
      // Chaque astéroïde a sa propre rotation aléatoire unique
      const rotVelX = (Math.random() - 0.5) * 0.006;
      const rotVelY = (Math.random() - 0.5) * 0.006;
      const rotVelZ = (Math.random() - 0.5) * 0.006;
      ...
      return {
        ...
        velocity: [velocityX, velocityY, velocityZ] as [number, number, number],
        rotationVelocity: [rotVelX, rotVelY, rotVelZ] as [number, number, number],
      };
    });
  }, [count]);

  // Initialize asteroid refs
  useEffect(() => {
    asteroidsRef.current = data.map(d => ({
      position: [...d.position],
      rotation: [...d.rotation],
      velocity: [...d.velocity],
      rotationVelocity: [...d.rotationVelocity],
    }));
  }, [data]);
  ...
  useFrame(() => {
    ...
    asteroidsRef.current.forEach((asteroid, i) => {
      ...
      // Update position with unique velocity for each asteroid
      ...
      const bounds = 50;
      if (asteroid.position[0] > bounds) asteroid.position[0] = -bounds;
      if (asteroid.position[0] < -bounds) asteroid.position[0] = bounds;
      if (asteroid.position[1] > bounds) asteroid.position[1] = -bounds;
      if (asteroid.position[1] < -bounds) asteroid.position[1] = bounds;
      if (asteroid.position[2] > bounds) asteroid.position[2] = -bounds;
      if (asteroid.position[2] < -bounds) asteroid.position[2] = bounds;

      // Update rotation with unique rotation velocity for each asteroid
      asteroid.rotation[0] += asteroid.rotationVelocity[0];
      asteroid.rotation[1] += asteroid.rotationVelocity[1];
      asteroid.rotation[2] += asteroid.rotationVelocity[2];

      // Apply to mesh
      children[i].position.set(...asteroid.position);
      children[i].rotation.set(...asteroid.rotation);
    });
  });
...
// --- Earth Planet ---
const Earth = ({ position = [40, 20, -60] }: { position?: [number, number, number] }) => {
      ...
      <meshPhysicalMaterial
        map={new THREE.CanvasTexture(
          (() => {
            ...
            ctx.fillRect(250, 80, 120, 140);
            ctx.fillRect(350, 200, 100, 80);
...
// --- Jupiter Planet ---
const Jupiter = ({ position = [-50, -15, -80] }: { position?: [number, number, number] }) => {
  const ref = useRef<THREE.Mesh>(null);
  ...
  return (
    <mesh ref={ref} position={position} scale={5}>
      <sphereGeometry args={[1, 64, 64]} />
      <meshPhysicalMaterial
        map={new THREE.CanvasTexture(
          (() => {
            ...
            ctx.fillRect(0, 380, 512, 50);
            
            // Light bands
            ctx.fillStyle = '#daa520';
            ctx.fillRect(0, 170, 512, 50);
            ctx.fillRect(0, 330, 512, 40);
            
            // Great Red Spot
            ctx.fillStyle = '#a0522d';
            ctx.beginPath();
            ctx.ellipse(350, 280, 60, 40, 0.3, 0, Math.PI * 2);
            ctx.fill();
...
const ShootingStars = () => {
  ...
  useFrame((state, delta) => {
    ...
    groupRef.current.children.forEach((star: any) => {
      ...
      if (
        Math.abs(star.position.x) > 300 ||
        Math.abs(star.position.y) > 300 ||
        Math.abs(star.position.z) > 300
      ) {
        star.position.set(
          (Math.random() - 0.5) * 400,
          (Math.random() - 0.5) * 400,
      ...
      // Fade effect
      star.material.opacity = Math.sin(state.clock.elapsedTime * 3 + star.userData.offset) * 0.5 + 0.5;
    });
  });
