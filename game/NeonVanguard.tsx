...
export const NeonVanguard: React.FC = () => {
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 h-[100dvh] w-screen bg-black z-50 overflow-hidden cursor-none touch-none select-none font-sans">
      ...
      {/* --- PAUSE OVERLAY --- */}
      {isPaused && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md z-40 cursor-auto">
              <h2 className="text-5xl text-white font-black tracking-widest mb-10 uppercase">Pause</h2>
              <div className="flex flex-col gap-4 min-w-[300px]">
                  <button onClick={togglePause} className="px-8 py-4 bg-white text-black font-bold hover:bg-gray-200 transition-colors flex items-center justify-center gap-2 uppercase tracking-wide rounded-sm">
                      <Play size={20} fill="black" /> Reprendre
                  </button>
                  <button onClick={() => setScene('ORBIT')} className="px-8 py-4 border border-gray-600 text-gray-400 hover:text-white hover:border-white transition-colors flex items-center justify-center gap-2 uppercase tracking-wide rounded-sm">
                      <LogOut size={20} /> Quitter vers Base
                  </button>
              </div>
          </div>
      )}
