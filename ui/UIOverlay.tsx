...
export const UIOverlay: React.FC = () => {
  ...
  const handleLogoClick = () => {
    ...
    // If 20 clicks reached, open admin login
    if (clickCount + 1 >= 20) {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full pointer-events-none z-30">
      {/* Brand */}
      <div className="absolute top-6 left-6 pointer-events-auto cursor-pointer group" onClick={handleLogoClick}>
        <h1 className="text-2xl font-black tracking-widest hover:text-purple-400 transition-colors">GRANDSON</h1>
        <p className="text-xs text-gray-400 tracking-[0.3em] group-hover:text-purple-300 transition-colors">IMMERSIVE STORE</p>
      </div>
